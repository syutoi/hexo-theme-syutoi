...
对应的主题即`Theme.Syutoi`，可以说是为了笔记阅读而生的主题。
...
修改站点配置文件`<root>/_config.yml`，把主题改为 `syutoi`
...
## 修改主题配置
主题配置的所有参数在`<root>/themes/syutoi/_config.yml`文件中。

为了方便主题升级，请在根目录新建一个yml文件，命名为`_config.syutoi.yml`。
也就是说，所有主题的自定义配置均保存于`<root>/_config.syutoi.yml`文件。
