...
# 自定义页面配色
主题配色全部在`<root>/themes/syutoi/source/css/_colors.styl`文件中，可以自行查看。
...
# 自定义主题图片
如果想要修改主题的`<root>/themes/syutoi/source/images/`目录内的某张图片，请在`<root>/source/_data/`目录新建目录`images`，并在这个文件夹中添加++同名++文件，部署时将自动覆盖主题内的默认图片。
