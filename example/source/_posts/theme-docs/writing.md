...
需要代码行号时，在博客 `_config.yml` 中开启 `highlight.line_number`；完整配置及长代码阈值见 [显示代码行号](/docs/configuration/#显示代码行号)。它是站点高亮配置，不需要逐篇添加 Front Matter。

代码围栏标注语言后在构建时高亮；不支持的语言可能显示为 plaintext。Tab 可聚焦代码滚动区域，左右方向键阅读长行。代码语言以小标签显示在面板内；有 Clipboard API 时提供复制图标（桌面悬停或键盘聚焦显示，触屏常显），保留缩进和换行，不复制行号。普通 HTTP 生产页面可能没有该 API；无 JS 或无 API 时仍可选择代码。复制失败会提供反馈，可重试。
