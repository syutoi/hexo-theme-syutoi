...
function normalizeConfig(theme = {}, overrides = {}) {
  ...
  const columnCategories = pick('columns', 'categories', 'all');
  ...
  return {
    ...
    columns: {
      categories: columnCategories === 'all' ? 'all' : [...new Set((Array.isArray(columnCategories) ? columnCategories : []).map(text).filter(Boolean))]
    },
    sidebar: {
