...
const socialNames = { github: 'GitHub', gitlab: 'GitLab', email: 'Email', twitter: 'Twitter', x: 'X', mastodon: 'Mastodon', bluesky: 'Bluesky', rss: 'RSS', website: 'Website' };
function socialLinks(value) {
  return links(socialEntries(value).map(entry => ({
    name: text(entry?.name) || socialNames[text(entry?.type).toLowerCase()] || '',
    url: entry?.url
  })));
}
