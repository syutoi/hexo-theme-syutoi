...
本页主要用于已有 Hexo 博客；新博客先看 [从空目录开始](#从空目录开始)，再执行安装步骤。想先查看效果，可按 [README](../README.md) 启动仓库自带示例。以下命令在博客根目录执行，要求 Node >=20.19.0、pnpm 9.0.4；当前实现以 Hexo 8.1.2 验证。本文对应当前开发分支，包版本仍为 0.4.0。木色视觉、文档中心及可选 Markdown 扩展属于 v0.4.0 标签之后的改动，使用旧标签时请阅读该标签内的文档；历史版本差异见 Changelog。
