...
当前版本为 `0.5.0`，收录木色视觉、田园配图、Demo 文档中心、可选 Markdown 扩展及安装/线上/性能验收；`0.4.0` 为 SEO/订阅、迁移文档与 G1–G4a 验证；`0.3.0` 为可选本地搜索与评论插槽；`0.2.0` 为阅读体验增强，`0.1.0` 为首个 MVP。此前 `0.2.5` 沿用自上游，不表示本项目已完成 PRD 的 0.2 阶段。版本与破坏性变更记录在 [CHANGELOG](../CHANGELOG.md)。`package.json` 版本也用于 JS/CSS URL 的缓存参数，修改后需重新生成示例站。
