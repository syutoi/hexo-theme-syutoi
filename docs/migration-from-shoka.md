...
本说明记录 B4–B6 已实施的改动；主题仍在重建，不代表完整 v0.1 已验收。上游历史、许可证和文章中的来源链接保持保留。
...
主题默认配置收敛到 111 行，示例覆盖为 24 行。只保留当前可用功能；示例不再继承上游作者的个人社交账号。历史文档中的原站链接与来源记录仍然保留。

配置兼容层区分站点覆盖与主题默认值，优先读取用户显式设置；新字段与旧别名同时出现时以新字段为准。列表整体替换，空列表不会恢复默认菜单。完整字段、优先级和迁移表见 [配置文档](configuration.md)。

## 下一步

B7 完善工程检查；完整排版、SEO、可访问性和性能预算继续按 [TODO](TODO.md) 验收。
